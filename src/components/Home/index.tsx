...
import { MainH2 } from '../MainH2'
import PortableTextRender from '../sanity/PortableTextRender'
import { FetchCarouselImage, HomeInfo } from './Carousel/FetchCarouselImage'
import { ContainerSectionMain } from './ContainerSectionMain'
import { FetchPosts } from './FetchPosts'

export async function HomeFech() {

   const tt = groq`*[_type=="home"][0]`;
   // home
   const yyy:HomeInfo = await client.fetch(tt);
  ...
  return (
    <div className="w-full flex flex-col gap-[clamp(20px,4vw,32px)] max-w-5xl m-auto relative h-full px-[min(4%_,_2rem)] py-12">
         ...
         <FetchCarouselImage ImageCarousel={yyy?.imageCarousel} />

         <ContainerSectionMain>
            <MainH2>{yyy?.productName}</MainH2>
            ...
            <PortableTextRender content={yyy?.blockContent} />
         </ContainerSectionMain>

               {/* Nossos  */}
         <ContainerSectionMain>
            <MainH2>
               {yyy?.titleSectionCard}
            </MainH2>
            {/* @ts-expect-error */}
            <FetchPosts />
         </ContainerSectionMain>
         ...
         <ContainerSectionMain>
            <MainH2>{yyy?.titleAddress}</MainH2>
